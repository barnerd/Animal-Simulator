...
public struct Vector2f {
    ...
    // added by DIB 11/27/19
    // I don't think takes which side of the object you're on into account
    public static bool Clockwise(Vector2f a, Vector2f b, Vector2f c)
    {
        return (b.y - a.y) * (c.x - b.x) - (b.x - a.x) * (c.y - b.y) > 0;
    }

    public static implicit operator Vector2(Vector2f p) => new Vector2((float)p.x, (float)p.y);
    public static explicit operator Vector2f(Vector2 p) => new Vector2f(p.x, p.y);
