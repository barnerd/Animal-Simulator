    ...
    internal class BeachLine
    {
        ...
        internal void AddBeachSection(FortuneSiteEvent siteEvent, MinHeap<FortuneEvent> eventQueue, HashSet<FortuneCircleEvent> deleted, LinkedList<VEdge> edges)
        {
            ...
            if (leftSection != null && leftSection == rightSection)
            {
                ...
                var y = ParabolaMath.EvalParabola(leftSection.Data.Site.Center.X, leftSection.Data.Site.Center.Y, directrix, x);
            ...
            {
                var start = new VPoint((leftSection.Data.Site.Center.X + site.Center.X)/ 2, double.MinValue);
            ...
            else if (leftSection != null && leftSection != rightSection)
            {
                ...
                var ax = leftSite.Center.X;
                var ay = leftSite.Center.Y;
                var bx = site.Center.X - ax;
                var by = site.Center.Y - ay;

                var rightSite = rightSection.Data.Site;
                var cx = rightSite.Center.X - ax;
                var cy = rightSite.Center.Y - ay;
                var d = bx*cy - by*cx;
        ...
        private static double LeftBreakpoint(RBTreeNode<BeachSection> node, double directrix)
        {
            ...
            //degenerate parabola
            if ((node.Data.Site.Center.Y - directrix).ApproxEqual(0))
                return node.Data.Site.Center.X;
            //node is the first piece of the beach line
            if (leftNode == null)
                return double.NegativeInfinity;
            //left node is degenerate
            if ((leftNode.Data.Site.Center.Y - directrix).ApproxEqual(0))
                return leftNode.Data.Site.Center.X;
            var site = node.Data.Site;
            var leftSite = leftNode.Data.Site;
            return ParabolaMath.IntersectParabolaX(leftSite.Center.X, leftSite.Center.Y, site.Center.X, site.Center.Y, directrix);
        }
        ...
        private static double RightBreakpoint(RBTreeNode<BeachSection> node, double directrix)
        {
            ...
            //left node is degenerate
            if ((rightNode.Data.Site.Center.Y - directrix).ApproxEqual(0))
                return rightNode.Data.Site.Center.X;
            var site = node.Data.Site;
            var rightSite = rightNode.Data.Site;
            return ParabolaMath.IntersectParabolaX(site.Center.X, site.Center.Y, rightSite.Center.X, rightSite.Center.Y, directrix);
        }
        ...
        private static void CheckCircle(RBTreeNode<BeachSection> section, MinHeap<FortuneEvent> eventQueue)
        {
            ...
            double bx = centerSite.Center.X,
                by = centerSite.Center.Y,
                ax = leftSite.Center.X - bx,
                ay = leftSite.Center.Y - by,
                cx = rightSite.Center.X - bx,
                cy = rightSite.Center.Y - by;
